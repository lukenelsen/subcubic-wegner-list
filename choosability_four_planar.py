...
#Identical to fChoosableNoPrint, except that it does not bother with attempting any pre-checks before the Nullstellsatz.  (We trust that the graph and f iare fed in correctly, and the minimum degree means no degeneracy reductions.)  (G_copy,f_copy input now G,f.)
def fChoosable_plantri_c3m4(G,f,inprocess=3,print_mod=1000,maxIS=True,LAscheme="L",rev=False):
