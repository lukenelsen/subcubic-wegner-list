...
# Section 5:  Verifying the Target Set
#    run_lemma_20
#    (main command)
...
# These Sage scripts were written with the intent of using Python 2.7 before the switch to Python 3.  There should not be many differences to implement our code in Python 3 once this change is made.
...
def core_square_graph(core_subgraph,stem_structure):
    ...
    # Now square the resulting graph.  (The current graph is H[A], where H is the realization and A is the set of core vertices.)
    M = new_G.distance_matrix()
    for i,j in combinations(range(new_G.order()),2):
        if M[i][j] == 2:
            new_G.add_edge((i,j))  # If (i,j) is already an edge of the graph, then this does nothing.
    ...
    # We need to make note of which roots are attached to stems of degree 1.
    roots = {x for x in core_subgraph.vertices() if core_subgraph.degree(x)==2}
    roots_already_observed = {x for stem_type in stem_structure for stem_group in stem_type for x in stem_group}
    one_stem_vtxs = roots - roots_already_observed    
    
    for v in core_subgraph.vertices():
        restriction_index = 0
        v_nbrs = set(core_subgraph.neighbors(v))  # v_nbrs will be the set of neighbors of v in H[A]...
        for edge in stem_structure[0]:  # ...so we add neighbors from root-to-root edges in the stem structure as well.
            v_nbrs |= edge
        # There is no need to remove v from v_nbrs if it was added.
        
        for one_stem in one_stem_vtxs:
        ...
        # If the last region has at least two roots, then we can try to form pairs or triples.  We do so conditioned on the last root.
            ...
            for two in combinations(range(len(roots)-1),2):
                ...
                for substructure in stem_structure_generator(new_roots_by_region):
                    structure = [[set(x) for x in y] for y in substructure]###try set
...
def check_all_realizations_of_configuration(config_str):
    ...
    # This simply prepares the natural core subgraph and feeds it into check_all_realizations_from_initial_plane_graph.  As the only routine which is not need for run_lemma_??.sage, this is separated from check_all_realizations_from_initial_plane_graph to distinguish from the preparations necessary for Lemma ??.
