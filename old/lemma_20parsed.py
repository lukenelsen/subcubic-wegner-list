...
# This file was *autogenerated* from the file lemma_20.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_7 = Integer(7); _sage_const_6 = Integer(6); _sage_const_60 = Integer(60); _sage_const_4 = Integer(4); _sage_const_100 = Integer(100); _sage_const_9 = Integer(9); _sage_const_10000 = Integer(10000); _sage_const_11 = Integer(11); _sage_const_10 = Integer(10); _sage_const_17 = Integer(17); _sage_const_15 = Integer(15); _sage_const_14 = Integer(14); _sage_const_20 = Integer(20); _sage_const_18 = Integer(18)# To-Do:
...
def timestring(time):
    ...
    t = int(time)
    d = str(int((_sage_const_10 *(time-t))%_sage_const_10 ))
    s = str(t%_sage_const_60 )
    m = int(t/_sage_const_60 )
...
class NaturalCoreSubgraph:
    ...
    def __init__(self,config_str):
        ...
        s = config_str[_sage_const_1 :config_str.index('a')]
        ...
        for s in config_str[config_str.index('a')+_sage_const_1 :]:
            if s == 'x':
        ...
        if self.is_open:
            for j in range(len(FLL)):  # Build the path from 0 to len(FLL) (inclusive).
                self.edges.append((j,j+_sage_const_1 ))
            self.faces.append(range(len(FLL)+_sage_const_1 ))  # Add the path to faces.
        ...
        # In the closed case, we build a cycle and append this to faces.
            ...
            for j in range(self.central_face_length-_sage_const_1 ):  # Build the path from 0 to self.central_face_length-1 (inclusive).
                self.edges.append((j,j+_sage_const_1 ))
            self.edges.append((_sage_const_0 ,self.central_face_length-_sage_const_1 ))  # Close the path to make it a cycle.
            self.faces.append(range(self.central_face_length))  # Add the cycle to faces.
            vtx = self.central_face_length-_sage_const_1   # vtx is the current vertex we are building edges to.  (For future use.)
        ...
        for i in range(len(FLL)):
            ...
            if i==_sage_const_0  or FLL[i-_sage_const_1 ]=='x':
                num_shared = _sage_const_0 
                start = i
            elif i>_sage_const_1  and FLL[i-_sage_const_1 ]==_sage_const_3  and FLL[i-_sage_const_2 ]!='x':  # This only occurs for cxa535.  We could have used the alternative encoding c3a55 but prefer the former.
                num_shared = _sage_const_2 
                start = vtx-_sage_const_1   # vtx-1 is the second-to-last vertex that was created for a face.  This is the right vertex because the face previous to the 3-face has length at least 5.
                new_face.append(vtx)
                new_face.append(vtx-_sage_const_1 )
            else:  # Previous entry is a specified face but not a sandwiched 3-face.
                num_shared = _sage_const_1 
            ...
            for j in range(FLL[i]-_sage_const_2 -num_shared):
                vtx += _sage_const_1 
            ...
            self.edges.append((i+_sage_const_1 ,vtx))
        ...
        self.spine = range(len(FLL)+_sage_const_1 )
...
def core_square_graph(core_subgraph,stem_structure):
    ...
    for edge in stem_structure[_sage_const_0 ]:  # Root-to-root edges from the stem structure.
    ...
    for i,j in combinations(range(new_G.order()),_sage_const_2 ):
        if M[i][j] == _sage_const_2 :
    ...
    for edge in stem_structure[_sage_const_1 ]:  # Roots connected to stems of degree 2 from the stem structure.
        new_G.add_edge(edge)  # If edge is already an edge of the graph, then this does nothing.
    for triple in stem_structure[_sage_const_2 ]:  # Roots connected to stems of degree 3 from the stem structure.
        for edge in combinations(triple,_sage_const_2 ):
            new_G.add_edge(edge)  # If edge is already an edge of the graph, then this does nothing.
    ...
    roots = {x for x in core_subgraph.vertices() if core_subgraph.degree(x)==_sage_const_2 }
...
def restricted_partition_generator(li,last_index,forbidden_dict):
    ...
    if last_index < _sage_const_0 :  # The relevant portion of li is empty if and only if last_index < 0.
        yield []
        return
    ...
    for partition in restricted_partition_generator(li,last_index-_sage_const_1 ,forbidden_dict):
...
def core_subgraph_generator(NCS,forbid_identifications={}):    
    ...
    if NCS.is_open:
        init_spec_face = _sage_const_1   # This will tell a later loop to skip the central face.
        ...
        forbidden_dict[NCS.spine[_sage_const_0 ]] |= set(NCS.spine[:_sage_const_3 ])
        forbidden_dict[NCS.spine[_sage_const_1 ]] |= set(NCS.spine[:_sage_const_4 ])
        forbidden_dict[NCS.spine[-_sage_const_2 ]] |= set(NCS.spine[-_sage_const_4 :])
        forbidden_dict[NCS.spine[-_sage_const_1 ]] |= set(NCS.spine[-_sage_const_3 :])
        # Now handle all middle vertices on the spine.
        for i in range(_sage_const_2 ,len(NCS.spine)-_sage_const_2 ):
            forbidden_dict[NCS.spine[i]] |= set(NCS.spine[i-_sage_const_2 :i+_sage_const_3 ])
        ...
        sandwiched_vertices = {x for x in range(_sage_const_1 ,len(NCS.facial_length_list)) if NCS.facial_length_list[x-_sage_const_1 ]!='x' and NCS.facial_length_list[x]!='x'}
        for v in NCS.spine:
        ...
        init_spec_face = _sage_const_0   # ...then we tell the next loop to include the central face.
    ...
    count = _sage_const_0   # Counts the number of partitions being checked.
    countCS = _sage_const_0   # Counts how many made it to be yielded as core subgraphs.
    ...
    for partition in restricted_partition_generator(range(NCS.order),NCS.order-_sage_const_1 ,forbidden_dict):
        count += _sage_const_1 
        if count % _sage_const_10000  == _sage_const_0 :  # Print every so often -- more helpful for large configurations.
            print "."*max(_sage_const_11 -len(str(count)),_sage_const_0 )+str(count)+" "*max(_sage_const_9 -len(str(countCS)),_sage_const_0 )+str(countCS)+"     "+timestring(time.clock()-begin)
        ...
        for e in NCS.edges:
            a = ident_dict[e[_sage_const_0 ]]
            b = ident_dict[e[_sage_const_1 ]]
        ...
        for x in range(len(partition)):
            degree_of_x = _sage_const_0 
            for e in new_edges:
                if x in e:
                    degree_of_x += _sage_const_1 
            if degree_of_x > _sage_const_3 :  # If we find a new vertex of degree more than 3, break immediately and continue to the next partition.
        ...
        new_end_1,new_end_2 = ident_dict[NCS.faces[_sage_const_0 ][_sage_const_0 ]],ident_dict[NCS.faces[_sage_const_0 ][-_sage_const_1 ]]
        ...
        new_faces.append([ident_dict[x] for x in NCS.faces[_sage_const_0 ]])  # Start with central face.
        central_face = new_faces[_sage_const_0 ]  # It will help readability to refer to 'central_face' instead of 'new_faces[0]'.
        ...
        for face in NCS.faces[_sage_const_1 :]:
            new_face = [ident_dict[x] for x in face]  # Convert to new vertex labels.
            for prev_face in new_faces[_sage_const_1 :]:  # We start comparison at index 1 to avoid comparing with the central face.  No specified face can be identified with the central face because the resulting face would contain a cut edge (the shared edge between the two faces in the natural core subgraph).  Since the face must also have a length at most 9, this violates Lemma ?.
        ...
        if NCS.is_open:
            for i,j in combinations(range(len(central_face)-_sage_const_1 ),_sage_const_2 ):
                if central_face[i] == central_face[j] and central_face[i+_sage_const_1 ] == central_face[j+_sage_const_1 ]:
                    some_condition_failed = True
                    break
            if some_condition_failed:
                continue
        ...
        for edge in J_prime.edges():  # The Graph edges iterator does not add edges part-way through the for loop.  Even though the edges of J_prime are changing over the course of the for loop, the objects in the iterator remain only those originally in J_prime when the for loop first started.
            J_prime.subdivide_edge((edge[_sage_const_0 ],edge[_sage_const_1 ]),_sage_const_1 )
            edge_dict[(edge[_sage_const_0 ],edge[_sage_const_1 ])] = i
            edge_dict[(edge[_sage_const_1 ],edge[_sage_const_0 ])] = i  # Keep track of both orientations of the edge -- we do not know which orientations we will need.
            i += _sage_const_1 
        ...
        for face in new_faces:
            ...
            for j in range(len(face)):
                ...
                J_prime.add_edge((edge_hub_vertex,face[j-_sage_const_1 ]))  # First endpoint.  Still works when j==0 due to Python's indexing.
                J_prime.add_edge((edge_hub_vertex,face[j]))  # Second endpoint.
                J_prime.add_edge((edge_hub_vertex,edge_dict[(face[j-_sage_const_1 ],face[j])]))  # Midpoint.
        ...
        if J_prime.is_planar():
            ...
            countCS += _sage_const_1 
            # Print info and yield J (not J_prime).
            print
            print "      >>> "+"Core Subgraph #%d"%(countCS),"(Partition #%d)"%(count)
            print " "*_sage_const_10 +"Partition: %s"%(str(partition))   
            # We print the edges ten to each line.
            ed = [e[:_sage_const_2 ] for e in J.edges()]
            ed_str_li = []
            rows = len(ed)/_sage_const_10 
            for x in range(rows):
                ed_str_li.append(str(ed[_sage_const_10 *x:_sage_const_10 *(x+_sage_const_1 )])[_sage_const_1 :-_sage_const_1 ]+",")
            ed_str_li.append(str(ed[_sage_const_10 *rows:])[_sage_const_1 :-_sage_const_1 ])
            print " "*_sage_const_10 +"Edges: %s"%(ed_str_li[_sage_const_0 ])
            for s in ed_str_li[_sage_const_1 :]:
                print " "*_sage_const_17 +s
            yield J,new_faces
...
def get_open_region_roots(graph,given_faces):
    ...
    all_directed_edges = {(e[_sage_const_0 ],e[_sage_const_1 ]) for e in graph.edges()} | {(e[_sage_const_1 ],e[_sage_const_0 ]) for e in graph.edges()}
    ...
    all_directed_edges |= {(given_faces[_sage_const_0 ][_sage_const_0 ],given_faces[_sage_const_0 ][-_sage_const_1 ]),(given_faces[_sage_const_0 ][-_sage_const_1 ],given_faces[_sage_const_0 ][_sage_const_0 ])}
    
    # Second, we remove all directed edges from given_faces (including across the spine ends).
    facial_directed_edges = {(face[j-_sage_const_1 ],face[j]) for face in given_faces for j in range(len(face))}
    # pointer_dict is a dictionary which assigns each tail of a remaining directed edge to the head of the directed edge.
    pointer_dict = {e[_sage_const_0 ]:e[_sage_const_1 ] for e in all_directed_edges-facial_directed_edges}
    ...
    while pointer_dict:
        v = pointer_dict.keys()[_sage_const_0 ]  # v is an arbitrary initial vertex.
    ...
    roots = {v for v in graph.vertices() if graph.degree(v)==_sage_const_2 }  # This includes the open spine ends, if there are any.
...
def stem_structure_generator(roots_by_region):
    ...
    if len(roots_by_region) == _sage_const_0 :
        yield [[],[],[]]
        return
    
    # Otherwise, examine the last region.
    else:
        roots = roots_by_region[-_sage_const_1 ]
        ...
        if len(roots) < _sage_const_2 :
            for substructure in stem_structure_generator(roots_by_region[:-_sage_const_1 ]):
                yield substructure
            return
        ...
        # If the last region has at least two roots, then we can try to form pairs or triples.  We do so conditioned on the last root.
            ...
            for two in combinations(range(len(roots)-_sage_const_1 ),_sage_const_2 ):
                # Break up the last region into three regions.
                new_roots_by_region = roots_by_region[:-_sage_const_1 ]
                new_roots_by_region.append(roots[:two[_sage_const_0 ]])
                new_roots_by_region.append(roots[two[_sage_const_0 ]+_sage_const_1 :two[_sage_const_1 ]])
                new_roots_by_region.append(roots[two[_sage_const_1 ]+_sage_const_1 :-_sage_const_1 ])
                ...
                for substructure in stem_structure_generator(new_roots_by_region):
                    ...
                    structure[_sage_const_2 ].append({roots[two[_sage_const_0 ]],roots[two[_sage_const_1 ]],roots[-_sage_const_1 ]})
                    yield structure
            
            # Case 2:  Condition on the last root being in a new edge.
            for one in range(len(roots)-_sage_const_1 ):
                # Break up the last region into two regions.
                new_roots_by_region = roots_by_region[:-_sage_const_1 ]
                new_roots_by_region.append(roots[:one])
                new_roots_by_region.append(roots[one+_sage_const_1 :-_sage_const_1 ])
                # For each substructure on new_roots_by_region, append our pair to the list of edges and yield.
                for substructure in stem_structure_generator(new_roots_by_region):
                    structure = [[set(x) for x in y] for y in substructure]
                    structure[_sage_const_0 ].append({roots[one],roots[-_sage_const_1 ]})
                    yield structure
            ...
            for one in range(len(roots)-_sage_const_1 ):
                ...
                for substructure in stem_structure_generator(new_roots_by_region):
                    structure = [[set(x) for x in y] for y in substructure]
                    structure[_sage_const_1 ].append({roots[one],roots[-_sage_const_1 ]})
                    yield structure
            
            # Case 4:  Condition on the last root being in a 1-stem (by itself).
            # Remove the last root from the last region.
            new_roots_by_region = roots_by_region[:-_sage_const_1 ]
            new_roots_by_region.append(roots[:-_sage_const_1 ])
...
def check_all_stem_structures_for_given_core_subgraph(graph,open_region_roots):
    ...
    begin = time.clock()
    count = _sage_const_0   # Total number of stem structures (and thus realizations) for this core subgraph
    good_count = _sage_const_0   # Number of core-choosable realizations
    bad_count = _sage_const_0   # Number of non-core-choosable realizations
    
    print " "*_sage_const_15 +"Index      Good   Bad   Time (Cum.)   Stem Structure"
    
    # Now we check them all.
    i = _sage_const_0   # i is the index of each stem structure, for printing purposes
    for SS in stem_structure_generator(open_region_roots):
        ...
        y = ch.fChoosableNoPrint(G_sq,f,inprocess=_sage_const_4 ,print_mod=_sage_const_100 ,maxIS=True,rev=False)
        if y[_sage_const_0 ]:
            good_count += _sage_const_1 
        else:
            bad_count += _sage_const_1 
        ti = timestring(time.clock()-begin)
        print " "*(_sage_const_20 -len(str(i)))+str(i)+" "*(_sage_const_10 -len(str(good_count)))+str(good_count)+" "*(_sage_const_6 -len(str(bad_count)))+str(bad_count)+" "*(_sage_const_14 -len(ti))+ti+"   "+str(SS)
...
def check_all_realizations_from_initial_plane_graph(NCS,forbid_identifications={}):
    ...
    # Initialize summary variables
    begin = time.clock()
    core_subgraph_count = _sage_const_0   # Counts the total number of core subgraphs
    realization_count = _sage_const_0   # Counts the total number of realizations
    bad_count = _sage_const_0   # Counts the total number of realizations which are not core-choosable
    ...
    for CS in core_subgraph_generator(NCS,forbid_identifications):
        ...
        open_region_roots = get_open_region_roots(CS[_sage_const_0 ],CS[_sage_const_1 ])
        
        # Check the stem structures
        good_stem_structure_count,bad_stem_structure_count = check_all_stem_structures_for_given_core_subgraph(CS[_sage_const_0 ],open_region_roots)
        
        # Update the summary info
        core_subgraph_count += _sage_const_1 
        ...
        totals_str += " "*(max(_sage_const_9 -len(core_subgraph_count_str),_sage_const_0 ))+core_subgraph_count_str+" "*(max(_sage_const_18 -len(stem_structure_count_str),_sage_const_0 ))+stem_structure_count_str+" "*(max(_sage_const_11 -len(bad_stem_structure_count_str),_sage_const_0 ))+bad_stem_structure_count_str+"\n"
    ...
    print " "*(max(_sage_const_9 -len(core_subgraph_count_str),_sage_const_0 ))+core_subgraph_count_str+" "*(max(_sage_const_18 -len(realization_count_str),_sage_const_0 ))+realization_count_str+" "*(max(_sage_const_11 -len(bad_count_str),_sage_const_0 ))+bad_count_str
    
    # Finally, print a concluding statement and total runtime
    if bad_count > _sage_const_0 :
